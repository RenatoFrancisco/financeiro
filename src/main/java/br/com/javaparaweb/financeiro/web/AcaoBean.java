...
public class AcaoBean {
	...
	public List<AcaoVirtual> getLista() {
		...
		try {
			if (this.lista == null) {
				this.lista = acaoRN.listarAcaoVirtual(contextoBean
						.getUsuarioLogado());
			}
		} catch (Exception e) {
	...
	public String getLinkCodigoAcao() {
		this.linkCodigoAcao = YahooFinanceUtil.getSiglaLink(this.selecionada
				.getAcao());
	...
	public PieChartModel getPercentualQuantidade() {
		...
		if (acoes.size() > 0) {
			...
			for (AcaoVirtual acaoVirtual : acoes) {
				...
				this.percentualQuantidade.set(acao.getSigla(),
						acao.getQuatidade());
			}
	...
	public PieChartModel getPercentualValor() {
		...
		if (acoes.size() > 0) {
			...
			for (AcaoVirtual acaoVirtual : acoes) {
				Acao acao = acaoVirtual.getAcao();
				this.percentualValor.set(acao.getSigla(), acao.getQuatidade());
			}
		}
